...
int main(int argc, char *argv[])
{
    ...
    // Semaphore setup
    sem_t *cook_queue = sem_open(COOK_SEM_NAME, O_CREAT, 0666, 0); /* Initialize with 0 */
    sem_t *sav_queue = sem_open(SAV_SEM_NAME, O_CREAT, 0666, 0); /* Initialize with 0 */
    sem_t *sem_mtx = sem_open(MTX_NAME, O_CREAT, 0666, 1); /* Initialize with 1 */
