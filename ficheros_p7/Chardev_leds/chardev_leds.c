...
#include <linux/slab.h>	/* for copy_to_user */
...
#define Scroll_Lock_ON 0x1
#define Num_Lock_ON 0x2
#define Caps_Lock_ON 0x4
...
static ssize_t device_write(struct file *filp, const char *buff, size_t len, loff_t * off)
{
    ...
    int status = ALL_LEDS_OFF, aux = len;
    while(--aux >= 0){
        if(str[aux]=='1')
            status |= Scroll_Lock_ON;
        else if(str[aux]=='2')
            status |= Num_Lock_ON;
        else if(str[aux]=='3')
            status |= Caps_Lock_ON;
